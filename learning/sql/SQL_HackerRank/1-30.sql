...
with CTE1 as
        (select *,
                case when brand1 > brand2 then concat(brand1, brand2, year)
                else concat(brand2, brand1, year) end as pair_id1
        from brands),
    cte_rn1 as        
    (select *,
    row_number(), over(partition by pair_id1 order by pair_id1)as rn1
    from CTE1)

select brand1, brand2, custom1, custom2, custom3, custom4, year 
from cte_rn1
where rn1 = 1
or (custom1 <> custom3 and custom2 <> custom4)
